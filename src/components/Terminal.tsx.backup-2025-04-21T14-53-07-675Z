...
const TerminalOutput = styled.div`
  margin-bottom: 16px;
  max-height: calc(100% - 60px); /* 增加空間給輸入區 */
  overflow-y: auto;
...
const Terminal: React.FC<TerminalProps> = ({ toggleTheme }) => {
  ...
  const getPrompt = () => {
    if (isSearchingHistory) {
      return `(reverse-i-search)\`${historySearchTerm}': `;
    }
  ...
  const addNewTab = () => {
    ...
    setActiveTabIndex(tabs.length); // 設置新標籤為活動標籤
  };
      ...
      <TabBar>
        {tabs.map((tab, index) => (
          ...
          >
            ...
            {tabs.length > 1 && (
              <CloseButton onClick={(e) => { e.stopPropagation(); closeTab(index); }}>
                ×
              </CloseButton>
            )}
          </Tab>
        ))}
        <AddTabButton onClick={addNewTab}>+</AddTabButton>
      </TabBar>
