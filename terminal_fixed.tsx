...
const Terminal: React.FC<TerminalProps> = ({ toggleTheme }) => {
  ...
  const [userName] = useState<string>('visitor');
  ...
  const [isFullFeatured] = useState<boolean>(false);
  ...
  const getPrompt = () => {
    ...
    // 根據當前標籤設置不同的用戶名和主機名
    const userName = currentTab.title === 'Terminal 2' ? 'dev' : 
                     currentTab.title === 'Terminal 3' ? 'admin' : 
                     'guest';
    const isRoot = currentTab.title === 'Terminal 3';
    const hostName = currentTab.title.toLowerCase().replace(' ', '');
    
    // 根據不同標籤返回不同風格的提示符
    const promptStyle = { 
      color: currentTab.title === 'Terminal 1' ? '#FF5722' : 
            currentTab.title === 'Terminal 2' ? '#2196F3' : 
            currentTab.title === 'Terminal 3' ? '#4CAF50' : 
            '#FF5722' 
    };
    
    if (isSudoPrompt) {
      return <span style={promptStyle}>[sudo] 密碼:</span>;
    }
    
    if (isRoot) {
      return <span style={promptStyle}>{userName}@{hostName}:<span style={{ color: '#03A9F4' }}>{currentDirectory}</span># </span>;
    }
  ...
  const addNewTab = (): CommandResult[] => {
    ...
    const newTab: TerminalTab = {
      ...
      cursorPosition: 0,
      isFullFeatured: true,
      userName: `guest-${newTabIndex + 1}`,
      isRoot: newTabIndex === 2, // 第三個標籤(索引2)是root
      aliases: {
        'll': 'ls -l',
        'la': 'ls -a',
        'cls': 'clear'
  ...
  return (
    <>
      ...
      <TabBar>
        {tabs.map((tab, index) => (
          <Tab 
            ...
            style={{ 
              backgroundColor: index === 0 ? '#2E2E2E' : 
                              index === 1 ? '#203746' : 
                              index === 2 ? '#243C2A' : '#2E2E2E'
            }}
          >
      ...
      <TerminalWrapper 
        ...
        style={{ 
          backgroundColor: activeTabIndex === 0 ? '#1E1E1E' : 
                          activeTabIndex === 1 ? '#0D2230' : 
                          activeTabIndex === 2 ? '#162918' : '#1E1E1E'
        }}
      >
